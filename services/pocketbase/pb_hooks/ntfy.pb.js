 ...
 *
 * JSVM SCOPING (critical): each handler callback runs in its OWN isolated
 * runtime, so module-scope helpers are NOT in scope inside it (runtime
 * "ReferenceError", never a build error). The `notify` helper is therefore
 * defined INSIDE each handler. See moderation.pb.js for the same constraint.
 */
